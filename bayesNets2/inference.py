...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # iterate over all variables in the elimination list given in order
        for var in eliminationOrder:
            # fetch the notJoined and joined factors
            factors,joinedFactor = joinFactorsByVariable(factors,var)
            # eliminate factor with more than one unconditional variables
            if len(joinedFactor.unconditionedVariables()) > 1 :
                factors+=[eliminate(joinedFactor,var)]
        # return normalized joined factor        
