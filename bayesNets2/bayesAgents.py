...
from layout import PROB_LEFT_TOP,PROB_OPPOSITE_CORNERS
...
def constructBayesNet(gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        #iterate over all house walls
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            # add all observation variables 
            obsVars+=[obsVar]
            # add domains of all observation variables
            variableDomainsDict[obsVar]=OBS_VALS
            # add edges from both houses to all observation variables
            edges+=[(HOUSE_VARS[0],obsVar),(HOUSE_VARS[1],obsVar)]
    
    
    # iterate over houses
    for house in HOUSE_VARS:
        # add edges from X pos to both houses
        edges+=[(X_POS_VAR,house),(Y_POS_VAR,house)]
        # add domain to both houses.
...
def fillYCPT(bayesNet, gameState):
    ...
    # set probability    
    yFactor.setProbability({Y_POS_VAR: BOTH_TOP_VAL}, PROB_LEFT_TOP * (1 - PROB_OPPOSITE_CORNERS))
    yFactor.setProbability({Y_POS_VAR: BOTH_BOTTOM_VAL},(1 - PROB_LEFT_TOP) * (1 - PROB_OPPOSITE_CORNERS) )
    yFactor.setProbability({Y_POS_VAR: LEFT_TOP_VAL},PROB_LEFT_TOP * PROB_OPPOSITE_CORNERS )
    yFactor.setProbability({Y_POS_VAR: LEFT_BOTTOM_VAL},(1 - PROB_LEFT_TOP) * PROB_OPPOSITE_CORNERS)
    # set Y-CPT
...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    centreX=gameState.data.layout.width/2
    centreY=gameState.data.layout.height/2
    
    # iterate over all possible house positions
    for housePos in gameState.getPossibleHouses():
        # iterate over all house wall positions
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            # construct factor
            obsFactor = bn.Factor([obsVar], HOUSE_VARS, bayesNet.variableDomainsDict())
            # top left position on the layout
            if obsPos[0] < centreX and obsPos[1] > centreY:
                posVal= TOP_LEFT_VAL
            # bottom left position on the layout 
            elif obsPos[0] < centreX and obsPos[1] < centreY:
                posVal= BOTTOM_LEFT_VAL
            # top right position on the layout
            elif obsPos[0] > centreX and obsPos[1] > centreY:
                posVal= TOP_RIGHT_VAL
            # bottom right position on the layout     
            else:
                posVal= BOTTOM_RIGHT_VAL
            
            # iterate over all possible assignments
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                probability=0
                # set probability based on house and the wall color
                if assignment[FOOD_HOUSE_VAR]==posVal:
                    if assignment[obsVar]== RED_OBS_VAL:
                        probability= PROB_FOOD_RED 
                    elif assignment[obsVar]== BLUE_OBS_VAL:
                        probability= 1- PROB_FOOD_RED
                elif assignment[GHOST_HOUSE_VAR]==posVal:
                    if assignment[obsVar]== RED_OBS_VAL:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    # generate factor p(FoodHouse | everything else) 
    factor= inferenceByVariableElimination(bayesNet,FOOD_HOUSE_VAR,evidence,eliminationOrder)
    # generate list of probability of all assignments
    listOfProbab=[factor.getProbability(assignment) for assignment in factor.getAllPossibleAssignmentDicts()]
    # return the assignment with maximum probability
    return factor.getAllPossibleAssignmentDicts()[listOfProbab.index(max(listOfProbab))]
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        # method to fetch left and right expected values
        def fetchProbabilty(assignment,factor):
            ...
            if assignment[FOOD_HOUSE_VAR]==TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR]==TOP_RIGHT_VAL:
                return ('left',probability*WON_GAME_REWARD + (1-probability) * GHOST_COLLISION_REWARD)
            elif assignment[FOOD_HOUSE_VAR]==TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR]==TOP_LEFT_VAL:
                return ('right',probability*WON_GAME_REWARD + (1-probability) * GHOST_COLLISION_REWARD)
        # generate factor by elimination       
        factor=inferenceByVariableElimination(self.bayesNet,HOUSE_VARS,evidence,eliminationOrder)
        # get a list storing left and right expected values by calling fetchProbabilty for each assignment
        expectedValue=[num for num in [fetchProbabilty(assignment,factor) for assignment in \
                                       factor.getAllPossibleAssignmentDicts()] if num!=None]
        # fetch left and right values from the list
        for val in expectedValue:
            if val[0]=='left':
                left=val[1]
            else:
                right=val[1]
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        listOfOutcomesAndProb=self.getExplorationProbsAndOutcomes(evidence)
        
        for probability,outcome in listOfOutcomesAndProb:
            for var in outcome:
                evidence[var]=outcome[var]            
            leftExpVal,rightExpVal=self.computeEnterValues(evidence, enterEliminationOrder)
            expectedValue+=(max(leftExpVal,rightExpVal)*probability)
